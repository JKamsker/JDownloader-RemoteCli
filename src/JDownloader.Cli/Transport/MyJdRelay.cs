...
internal static class MyJdParameterMapper
{
    ...
    private static (object? Parameters, IReadOnlyList<string>? Warnings) BuildConfigListParameters(object? query, out IReadOnlyList<string>? warnings)
    {
        ...
        var pattern = values.TryGetValue("pattern", out var rawPattern)
            ? rawPattern?.ToString() ?? string.Empty
            : string.Empty;

        var returnDescription = ReadOptionalBool(values, "returnDescription");
        var returnValues = ReadOptionalBool(values, "returnValues");
        var returnDefaultValues = ReadOptionalBool(values, "returnDefaultValues");
        var returnEnumInfo = ReadOptionalBool(values, "returnEnumInfo");
    ...
    {
        warnings = null;
        if (query is not Dictionary<string, object?> values || !TryGetLong(values, ["id"], out var id))
        ...
        return (new object?[] { id, format }, null);
    }
    ...
    private static (object? Parameters, IReadOnlyList<string>? Warnings) BuildCaptchaSolveParameters(object? query, out IReadOnlyList<string>? warnings)
    {
        ...
        if (string.IsNullOrWhiteSpace(resultFormat))
            return (new object?[] { id, result }, null);

        return (new object?[] { id, result, resultFormat }, null);
    }
    ...
    {
        warnings = null;
        if (query is not Dictionary<string, object?> values || !TryGetLong(values, ["id", "jobId", "jobid"], out var id))
    ...
    private static (object? Parameters, IReadOnlyList<string>? Warnings) BuildCaptchaForwardCreateJobParameters(object? query, out IReadOnlyList<string>? warnings)
    {
        warnings = null;
        if (query is not Dictionary<string, object?> values
            || !TryGetString(values, ["arg1"], out var arg1)
            || !TryGetString(values, ["arg2"], out var arg2)
            || !TryGetString(values, ["arg3"], out var arg3)
            || !TryGetString(values, ["arg4"], out var arg4))
        {
    ...
    {
        warnings = null;
        if (query is not Dictionary<string, object?> values || !TryGetLong(values, ["subscriptionid", "subscriptionId"], out var id))
    ...
    private static (object? Parameters, IReadOnlyList<string>? Warnings) BuildEventsSubscribeParameters(object? query, out IReadOnlyList<string>? warnings)
    {
        ...
        var subscriptions = values.TryGetValue("subscriptions", out var rawSubscriptions) ? ToStringList(rawSubscriptions) : [];
        var exclusions = values.TryGetValue("exclusions", out var rawExclusions) ? ToStringList(rawExclusions) : [];
    ...
    private static bool TryGetLong(Dictionary<string, object?> values, string[] keys, out long longValue)
    {
        foreach (var key in keys)
        {
            if (values.TryGetValue(key, out var rawValue) && rawValue is not null && TryReadLong(rawValue, out longValue))
                return true;
        }
    ...
    private static bool TryGetString(Dictionary<string, object?> values, string[] keys, out string value)
    {
        foreach (var key in keys)
        {
    ...
    private static bool ReadOptionalBool(Dictionary<string, object?> values, string key)
    {
        if (!values.TryGetValue(key, out var rawValue) || rawValue is null)
            return false;

        if (TryReadBool(rawValue, out var boolValue))
            return boolValue;
